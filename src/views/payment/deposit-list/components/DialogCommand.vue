      ...
      el-row.flex-wrap(type="flex" :gutter="20")
        el-col(:span="12")
          el-form-item
            ...
            el-input(v-model="info.internalRemark" type="textarea" :rows="10" disabled)
        el-col(:span="12")
          el-form-item
            FormItemLabel {{ `${$t('externalNoteRecord')}:` }}
            el-input(v-model="info.externalRemark" type="textarea" :rows="10" disabled)
      el-row.flex-wrap(type="flex" :gutter="20")
        el-col(:span="12")
          el-form-item
            FormItemLabel {{ `${$t('addInternalNoteRecord')}:` }}
            el-input(v-model="notes.internalRemark" type="textarea" :rows="10")
        el-col(:span="12")
          el-form-item
            FormItemLabel {{ `${$t('addExternalNoteRecord')}:` }}
            el-input(v-model="notes.externalRemark" type="textarea" :rows="10")
...
import {
  getPaymentDepositDetailsUuid,
  patchPaymentDepositPaidUuid,
  patchPaymentDepositDeclineUuid,
  patchPaymentDepositNoteUuid,
} from '@/api/payment';
export default {
  name: 'DialogCommand',
  components: {
  ...
  data() {
    return {
      ...
      notes: {},
      apiMapping: {
        confirm: patchPaymentDepositPaidUuid,
        reject: patchPaymentDepositDeclineUuid,
        remark: patchPaymentDepositNoteUuid,
      },
    };
  },
  computed: {
